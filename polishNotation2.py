...
def mergeEmptyInitializers(theList):
    newList = []
    for current in theList:
        if current == "[]" and newList != []:
            newList[-1] += "[]"
        else:
            newList += [current]
    return newList

def tokenizeString(aString):
    separators = ["..", "{}", "[]", "][", "==", ",", "!=", ">=", "<=", "+=", "*=", "}", "{", "]", "[", "--", "++", "!=", "^", "=", "-=", "*", "/", "+", "-", ";", ":", "#", "\n", ".", "//"]
    ...
    while i < len(aString):
        ...
        if theSeparator != "":
            ...
            i += 1
    return mergeEmptyInitializers(listToReturn)
...
def evaluateMacro(theOldArray, reallyNewInfoArray):
    ...
    if len(theOldArray) == 1:
            if type(theOldArray[0]) != str:
                return evaluateMacro(theOldArray[0], reallyNewInfoArray)
            else:
                if theOldArray[0] in ["]", "}"]:
                    return theOldArray[0]+"\n"
                else:
                    return theOldArray[0]
    ...
    for current in listOfSeparators():
        if(current == "^"):
            newCurrent = "**"
        else:
            newCurrent = current
        ...
        if theOldArray[0] == current:
            for i in range(1, len(theOldArray)):
                theOldArray[i] = evaluateMacro(theOldArray[i], reallyNewInfoArray)
            return "(+ " + " ".join(theOldArray[1:len(theOldArray)]) + ")"
            
            #raise Exception("It starts with " + current + ". Find out what to do next.")
    ...
    #        if(joinedOutput[current] != joinedOutput[current-1]):
        if theOldArray[0] in ["print", "return"]:
            return evaluateMacro([theOldArray[0]] + [theOldArray[1:len(theOldArray)]], reallyNewInfoArray)
        if (theOldArray[0] in ["if", "elsif", "elif", "while", "until"]):
            if "then" in theOldArray:
                return evaluateMacro([theOldArray[0]] + [theOldArray[1:theOldArray.index("then")]] + theOldArray[theOldArray.index("then"):len(theOldArray)], reallyNewInfoArray)
            elif ":" in theOldArray:
                return evaluateMacro([theOldArray[0]] + [theOldArray[1:theOldArray.index(":")]] + theOldArray[theOldArray.index(":"):len(theOldArray)], reallyNewInfoArray)
        if theOldArray[1] in ["+=", "-=", "\=", "=", ":=", "^=", "*="]:
            return evaluateMacro(theOldArray[0:2] + [theOldArray[2:len(theOldArray)]], reallyNewInfoArray)
        if len(theOldArray) > 2 and theOldArray[0]:
            if theOldArray[2] in ["="]:
                return evaluateMacro(theOldArray[0:3] + [theOldArray[3:len(theOldArray)]], reallyNewInfoArray)
...
def makeReallyNewInfoArray(theInfoArray, theLanguage):
    import traceback
    #import sys
    #raise Exception("reallyNewInfoArray")
    print("calling makeReallyNewInfoArray")
    ...
    #print("Length of reallyNewInfoArray:", len(reallyNewInfoArray))
